...
//	@PutMapping("/auth/updateroomstatus/{id}")
	public ResponseEntity<?> updateGuestDetails(@PathVariable Integer id, @RequestBody GuestDetails guestDetails) {
		// Validate input
		...
		try {
			...
			existingGuestDetails.setGuestDetailsStats(guestDetails.getGuestDetailsStats());
...
//	@DeleteMapping("/auth/deleteroomstatus/{id}")
	public ResponseEntity<String> deleteGuestDetail(@PathVariable("id") int id) {
		boolean isDeleted = service.deleteGuestDetail(id);
		if (isDeleted) {
