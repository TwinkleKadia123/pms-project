...
//	@PutMapping("/auth/updateroomstatus/{id}")
	public ResponseEntity<?> updateRentDetails(@PathVariable Integer id, @RequestBody RentDetails rentDetails) {
		// Validate input
...
//	@DeleteMapping("/auth/deleteroomstatus/{id}")
	public ResponseEntity<String> deleteRentDetail(@PathVariable("id") int id) {
		boolean isDeleted = service.deleteRentDetail(id);
		if (isDeleted) {
